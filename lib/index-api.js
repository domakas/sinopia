...
module.exports = function(config, auth, storage) {
  ...
  app.get('/packages.json', can('access'), function(req, res, next) {
    storage.get_all_local(function(err, packages) {
      ...
      packages.map(function(package) {
        ...
        for (var prop in package.versions) {
          if(!package.hasOwnProperty(prop)) {
            currentPackage = package.versions[prop];
            versions[prop] = {
              name: currentPackage.name,
              version: currentPackage.version,
              dist: {
                url: currentPackage.dist.tarball.replace('http://', 'https://'),
                type: 'tar',
                shasum: currentPackage.dist.shasum
              }
            };

            if (currentPackage.repository) {
              versions[prop].source = {
                'type': currentPackage.repository.type,
                'url': currentPackage.repository.url,
                'reference': currentPackage.gitHead
              };
            }
          }
        }

        result[package.name] = versions;
      });
